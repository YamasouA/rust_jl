...
fn bind_mount(root: &str) {
    for d in BIND_DIRS {
        let sd = format!("/{}", d); // バインドされる元
	let rd = format!("{}/{}", root, d); // バインドする方 rootは新しいマウントポイント
	...
	if sdm.file_type().is_symlink() { // 見ているファイルがシンボリックリンクなら
	    ...
	    if fs::read_dir(&rd).unwrap().count() == 0 { // ディレクトリが空なら
	        libmount::BindMount::new(Path::new(&sd), Path::new(&rd)) // Path::new -> string slice to Path slice
	            .recursive(false)
	    	.readonly(true)
	    	.mount()
